...
public class Graph {
    ...
    public Graph(String outputDirectory){
        graph = new ArrayList<>();
    ...
    public void setAssemblyPoints(List<Integer> assemblyPointsIndices, List<String> points) {
        vertices.clear();
        for (Integer index : assemblyPointsIndices) {
            vertices.add(new Vertex(points.get(index)));
        }
    }

    public void createEdgesFromMatrix() {
        for (int i = 0; i < adjacencyMatrix.length; i++) {
            for (int j = 0; j < adjacencyMatrix[i].length; j++) {
                if (adjacencyMatrix[i][j] != 0) {
                    Vertex origin = new Vertex(Integer.toString(i));
                    Vertex destination = new Vertex(Integer.toString(j));
                    graph.add(new Edge(origin, destination, adjacencyMatrix[i][j]));
                }
            }
        }
    }
    ...
    public void generateGraph(String filename, List<Vertex> assemblyPoints) {
        try {
